...
export const forumApi = {
  ...
  // 获取最新主题列表
  getLatestTopics(page = 1, pageSize = 10): Promise<{ count: number; next: string | null; previous: string | null; results: Topic[] }> {
    return request.get('/topics/', { params: { page, page_size: pageSize, ordering: '-created_at' } })
  },

  // 搜索话题
  searchTopics(query: string, page = 1, pageSize = 10): Promise<{ count: number; next: string | null; previous: string | null; results: Topic[] }> {
    return request.get('/topics/', { params: { search: query, page, page_size: pageSize, ordering: '-created_at' } })
  },

  // 搜索帖子
  searchPosts(query: string, page = 1, pageSize = 10): Promise<{ count: number; next: string | null; previous: string | null; results: Post[] }> {
    return request.get('/posts/', { params: { search: query, page, page_size: pageSize, ordering: '-created_at' } })
  },
  ...
  // 获取帖子评论列表
  getComments(postId: number): Promise<{ results: Comment[] }> {
    return request.get('/comments/', { params: { post: postId } })
  },

  // 举报评论
  reportComment(commentId: number, data: { report_type: string; description: string }): Promise<void> {
    return request.post(`/comments/${commentId}/report/`, data)
  },

  // 获取热门标签
  getHotTags(): Promise<{ id: number; name: string; topic_count: number }[]> {
    return request.get('/tags/hot/')
  },

  // 获取标签详情
  getTag(tagId: number): Promise<{ id: number; name: string; topic_count: number }> {
    return request.get(`/tags/${tagId}/`)
  },

  // 获取标签下的主题
  getTagTopics(tagId: number, page = 1, pageSize = 10): Promise<{ count: number; next: string | null; previous: string | null; results: Topic[] }> {
    return request.get(`/tags/${tagId}/topics/`, { params: { page, page_size: pageSize } })
  }
} 