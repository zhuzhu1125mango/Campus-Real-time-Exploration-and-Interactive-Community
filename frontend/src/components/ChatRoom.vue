      ...
      <div v-for="(message, index) in messages" :key="index" 
        ...
        <template v-else>
          <div class="message-avatar" @click="showUserMenu(message.user)">
              <img 
                :src="message.user?.avatar || `${config.media.baseUrl}${config.media.defaultAvatar}`" 
    ...
    >
      <div class="user-menu-header">
        <div class="user-menu-avatar">
          <img :src="currentUser?.avatar || `${config.media.baseUrl}${config.media.defaultAvatar}`" :alt="currentUser?.username">
        </div>
...
import { formatMessageContent } from '../utils/xss'
...
const connectWebSocket = async () => {
  ...
  if (!validToken) {
    console.log('[ChatRoom] 无法获取有效 token，跳过 WebSocket 连接')
    connectionStatus.value = 'disconnected'
  ...
  try {
    ...
    console.log('[ChatRoom] WebSocket URL:', wsUrl);
    ...
    ws.value.onclose = (event) => {
      console.log('[ChatRoom] WebSocket 连接已关闭，代码:', event.code, '原因:', event.reason)
      connectionStatus.value = 'disconnected'
      ...
      if (reconnectAttempts.value < maxReconnectAttempts && userStore.isLoggedIn) {
        ...
        console.log(`[ChatRoom] 将在 ${delay}ms 后尝试重新连接，第 ${reconnectAttempts.value} 次`)
...
const handleImageError = (event: Event) => {
  ...
  target.src = `${config.media.baseUrl}${config.media.defaultAvatar}`;
  target.alt = '默认头像';
};
...
const addFriend = async () => {
  ...
  // 保存用户名，防止请求过程中 currentUser 被清空
  const username = currentUser.value.username;
  
  try {
    // 直接使用 axios 发送请求，确保正确发送 POST 请求
    const token = localStorage.getItem(config.jwt.accessTokenKey);
    console.log('发送好友请求:', { receiverId: currentUser.value.id, tokenExists: !!token });
    ...
    if (errorData) {
      ...
      } else if (errorData.receiver && errorData.receiver[0]) {
        errorMessage = errorData.receiver[0];
      } else if (Array.isArray(errorData) && errorData[0]) {
