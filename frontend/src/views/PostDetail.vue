<template>
  <div class="post-detail-container">
    ...
    <div v-else-if="post" class="post-content-container">
      <div class="post-header">
        ...
        <div class="post-meta">
          ...
          <div class="post-actions-bar">
            ...
            <!-- 编辑和删除按钮（仅作者可见） -->
            <div v-if="isAuthor" class="post-management">
      ...
      <div v-if="!editingPost" class="post-content">
        {{ post.content }}
      </div>
      ...
      <div v-else class="edit-form">
        ...
        <div class="edit-actions">
          ...
          >
            <span v-if="submittingEdit" class="loading-spinner"></span>
            <span>{{ submittingEdit ? '保存中...' : '保存修改' }}</span>
          </button>
        </div>
      </div>
...
const isAuthor = computed(() => {
  ...
  return currentUserId === String((post.value as any).author.id)
})
...
const fetchComments = async () => {
  ...
  try {
    const response = await forumApi.getComments(post.value.id)
...
const saveEdit = async () => {
  ...
  try {
    await forumApi.updatePost(post.value.id, { content: editContent.value.trim() })
    
    // 更新本地数据
    post.value.content = editContent.value.trim()
    editingPost.value = false
    editContent.value = ''
...
.icon-edit {
  background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%234361ee"><path d="M11.5 4.02V20.98c0 .55-.45 1-1 1s-1-.45-1-1V4.02c0-.55.45-1 1-1s1 .45 1 1zm3.21-2c-.45 0-.81.36-.81.81V20.98c0 .45.36.81.81.81.45 0 .81-.36.81-.81V2.83c0-.45-.36-.81-.81-.81zm3.28 0c-.45 0-.81.36-.81.81V20.98c0 .45.36.81.81.81s.81-.36.81-.81V2.83c0-.45-.36-.81-.81-.81z"/></svg>');
}
